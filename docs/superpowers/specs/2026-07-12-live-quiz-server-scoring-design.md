...
4. `submit-answer` (rôle `service_role`, jamais exposé au client) : lit `session_quiz_answers` pour cette question, calcule correct/faux + points (les 7 types réellement jouables aujourd'hui — voir note ci-dessous), calcule le bonus de vitesse à partir de `question_started_at` (horloge serveur, pas une valeur envoyée par le client), fait l'upsert atomique dans `session_state.players`.
5. L'Edge Function renvoie `{ correct, earnedPoints, correctAnswer }` (le corrigé, révélé seulement en réponse à la soumission — jamais avant, comme aujourd'hui). **Correction par rapport à la première version de ce spec** : un preview décoratif correct/faux instantané n'est plus possible une fois le corrigé retiré de `quiz_data` — le client n'a plus aucune base locale pour deviner la correction (c'est justement l'objectif du fix). Dès le clic, le client affiche un état neutre "réponse envoyée, vérification..." (spinner, réponse verrouillée immédiatement, pas de changement possible) ; au retour de l'Edge Function (typiquement <300ms), l'UI affiche correct/faux + points — un tout petit délai remplace l'ancien feedback à latence zéro, mais c'est le prix direct et nécessaire de ne plus exposer le corrigé côté client.
...
3. Calcule `correct` — logique portée fidèlement de `PlayerView.tsx:447-478` pour les 7 types réellement jouables aujourd'hui (`multiple-choice`/`single-choice`, `true-false`, `short-answer`, `slider`, `fill-blank`, `ranking`, `matching`).

   **Note sur `drag-drop`/`hotspot`** : déclarés dans `questionTypes.ts` et le sélecteur de type du builder (`QuestionTypeSelector.tsx`), mais sans éditeur de question, sans rendu joueur et sans capture de réponse nulle part dans le code — un host ne peut pas en créer, un joueur ne peut pas en recevoir. Ce n'est pas juste "la correction manquante", c'est une fonctionnalité jamais construite. Hors périmètre de ce plan (nécessiterait éditeur + interaction glisser-déposer + rendu joueur + capture de réponse — un projet à part entière). `submit-answer` ne gère que les 7 types ci-dessus ; si une question `drag-drop`/`hotspot` venait à exister dans `session_quiz_answers` (ne devrait jamais arriver vu qu'aucune UI ne permet de la créer), la fonction renvoie une erreur explicite plutôt que de planter silencieusement.
...
- **Unit** (Edge Function) : les 7 types de correction avec cas limites (réponse vide/null, casse/espaces sur `short-answer`, ordre partiel sur `ranking`, blanks incomplets sur `fill-blank`) ; idempotence (2e soumission même question) ; bonus vitesse à `t=0`, `t=timeLimit`, après `timeLimit`.
