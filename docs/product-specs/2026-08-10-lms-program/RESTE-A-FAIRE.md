...
- [x] `follow_up_tasks` (moitié — création manuelle, pas encore le déclenchement automatisé) — `20260813050000_follow_up_tasks.sql`. AUT-002 en fait une action d'automatisation, mais `automation_rule_versions` (config des actions) n'a aucun writer nulle part et `record_automation_run()` aucun appelant — le moteur d'exécution des automatisations est lui-même dormant, chantier bien plus gros que cet item seul. Ce qui était scopé : aucune policy d'insertion n'existait du tout sur la table. `create_follow_up_task()` (staff `trainer`/`pedago`/`admin`, assigné vérifié membre de l'org). UI `Analytics.tsx` : bouton « Créer une tâche de suivi » par signal de risque (ferme la boucle que `resolve_risk_signal()` laisse volontairement ouverte — « human-in-the-loop, aucune action automatique ne suit »), plus `FollowUpTasksPanel` (file ouverte, RLS scope déjà qui voit quoi — formateur ne voit que ses tâches, pedago/admin voient toute la file de l'org). Transition de statut sans RPC (`follow_up_tasks_manage` permet déjà l'écriture directe). **Reste** : déclenchement automatique depuis une règle d'automatisation (nécessite le moteur d'exécution, non construit)
