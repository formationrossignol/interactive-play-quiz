...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "SolutionsEventsPage" });
  return {
    title: t("meta.title"),
    description: t("meta.description"),
    alternates: { canonical: getLocalizedAlternates("/solutions/events").languages[locale], ...getLocalizedAlternates("/solutions/events") },
  };
}
...
export default async function EventsSolutionPage({ params }: Props) {
  ...
  return <AuthorityPage
    ...
    layout={t("layout") as "stage"}
    tone={t("tone") as "amber"}
    eyebrow={t("eyebrow")}
    title={t("title")}
    accent={t("accent")}
